...
interface ContactProps {
  ...
  onAddConsultation: (consultation: { name: string; phone: string; message: string }) => void;
}

export default function Contact({ contactData, onAddConsultation }: ContactProps) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-stretch">
              ...
              <div className="space-y-5">
                ...
                <div className="flex items-start space-x-3.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-300 mt-1">{contactData.email}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-300 mt-1">{contactData.workingHours}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-7 bg-[#fbf9f5] border border-[#dfd4c0] rounded-2xl p-6 sm:p-8 shadow-sm flex flex-col justify-between">
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <h3 className="font-serif text-lg font-bold text-[#153020] mb-1">THÔNG TIN NHẬN TƯ VẤN</h3>
                <p className="text-slate-500 text-xs font-sans">
                  Để lại thông tin và cầu hỏi để được tư vấn miễn phí nhanh nhất.
                </p>
              </div>
              ...
              {isSuccess && (
                <div className="bg-emerald-50 text-emerald-800 text-xs px-4 py-3 rounded-lg border border-emerald-200 flex items-center space-x-3 font-sans">
                  ...
                  <span>Lời nhắn của khách hàng đã được tiếp nhận, Thảo Dược Hương Vũ sẽ liên hệ bạn sớm nhất có thể.</span>
                </div>
              )}
